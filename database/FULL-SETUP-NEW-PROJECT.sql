-- =========================================================
-- Reverse Tech - Full Setup Script
...
create table if not exists public.mechanical_parts (
  ...
  manufacturing_time text default '',
  active boolean not null default true,
  created_at timestamptz not null default now()
);
alter table public.mechanical_parts enable row level security;

create table if not exists public.mechanical_orders (
  id uuid primary key default gen_random_uuid(),
  part_id uuid references public.mechanical_parts(id) on delete set null,
...
alter table public.mechanical_orders enable row level security;

-- 4. 3D Printing Orders
...
insert into storage.buckets (id, name, public, file_size_limit)
values ('mechanical-images', 'mechanical-images', true, 10485760)
on conflict (id) do update set public = true, file_size_limit = 10485760;
...
('p1','project1.png','هندسة عكسية','Reverse Engineering','إعادة تصنيع قطعة غيار لجهاز أشعة','Reverse-Engineered X-Ray Machine Spare Part','مسح ثلاثي الأبعاد وإعادة تصميم قطعة غيار أصلية لجهاز أشعة طبي.','3D scanning and redesign of an original spare part for a medical X-ray machine.',1),
('p2','project2.png','طباعة ثلاثية الأبعاد','3D Printing','نمذجة ثلاثية الأبعاد لمكونات جهاز تنفس','3D Modeling of Ventilator Components','تصميم وطباعة مكونات بلاستيكية دقيقة لجهاز تنفس صناعي.','Design and printing of precise plastic components for an industrial ventilator.',2),
('p3','project3.jpg','تصنيع إلكتروني','Electronic Manufacturing','تصنيع لوحة إلكترونية لجهاز مراقبة القلب','PCB Manufacturing for a Cardiac Monitor','تصميم وتصنيع لوحة PCB بديلة لجهاز مراقبة القلب مع فحص كامل للأداء.','Design and manufacturing of a replacement PCB for a cardiac monitor.',3),
('p4','project4.jpg','صيانة ومعايرة','Maintenance & Calibration','معايرة جهاز تحليل الدم','Blood Analysis Device Calibration','صيانة ومعايرة دورية لجهاز تحليل دم بمستشفى شريك وفق المعايير الدولية.','Routine maintenance and calibration of a blood analysis device at a partner hospital.',4),
('p5','project5.jpg','طباعة ثلاثية الأبعاد','3D Printing','طباعة هيكل خارجي لجهاز طبي محمول','Housing Printed for a Portable Medical Device','إعادة تصميم وطباعة هيكل خارجي متين لجهاز طبي محمول.','Redesign and printing of a durable outer housing for a portable medical device.',5),
...
-- Default PCB pricing
insert into public.pcb_pricing (id, config) values (1, '{"pricePerCm2ByLayers":{"1":1.2,"2":2.2,"4":6.5,"6":11},"standardThicknessMm":1.6,"colorSurchargePercent":15,"thicknessSurchargeEGP":60,"setupFeeEGP":150,"minimumOrderEGP":250,"rushSurchargePercent":30,"shippingFees":{"pickup":0,"cairo":80,"giza":80,"other":120},"quantityDiscountTiers":[{"minQty":1,"discountPercent":0},{"minQty":10,"discountPercent":5},{"minQty":50,"discountPercent":10},{"minQty":100,"discountPercent":15},{"minQty":500,"discountPercent":22}],"discountCode":{"code":"REVERSE10","discountPercent":10,"active":true},"estimatedLeadTime":{"standard":"أسبوعين إلى ثلاثة أسابيع","rush":"أسبوع واحد (شحن/تصنيع عاجل)"}}'::jsonb)
on conflict (id) do update set config=excluded.config, updated_at=now();

-- Default graduation support config
insert into public.graduation_support_config (id, config)
values (1, '{"start_date":"2026-08-01","end_date":null,"closed_message_ar":"انتهت فترة التسجيل لدعم مشاريع التخرج لهذا الموسم، تابعونا لمعرفة موعد الفتح القادم","closed_message_en":"Registration for this seasons graduation project support has closed. Stay tuned for the next opening."}'::jsonb)
on conflict (id) do update set config = excluded.config, updated_at = now();
